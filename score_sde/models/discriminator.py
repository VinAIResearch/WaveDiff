...
class WaveletDownConvBlock(nn.Module):
    def __init__(
        self,
        in_channel,
        out_channel,
...
class WaveletDiscriminator_small(nn.Module):
    ...
    def __init__(self, nc = 3, ngf = 64, t_emb_dim = 128, act=nn.LeakyReLU(0.2), patch_size=1, use_local_loss=False, num_layers=4):
        ...
        # Encoding layers where the resolution decreases
        self.start_conv = conv2d(nc,ngf*2,1, padding=0)
        self.conv1 = WaveletDownConvBlock(ngf*2, ngf*2, t_emb_dim = t_emb_dim,act=act)
        
        self.conv2 = WaveletDownConvBlock(ngf*2, ngf*4,  t_emb_dim = t_emb_dim, downsample=True,act=act)
...
class WaveletDiscriminator_large(nn.Module):
    """A time-dependent discriminator for large images (CelebA, LSUN)."""
    ...
    def __init__(self, nc = 1, ngf = 32, t_emb_dim = 128, act=nn.LeakyReLU(0.2), patch_size=1, use_local_loss=False, num_layers=6):
        ...
        self.start_conv = conv2d(nc,ngf*2,1, padding=0)
        self.conv1 = WaveletDownConvBlock(ngf*2, ngf*4, t_emb_dim = t_emb_dim, downsample = True, act=act)
        
        self.conv2 = WaveletDownConvBlock(ngf*4, ngf*8,  t_emb_dim = t_emb_dim, downsample=True,act=act)
        ...
        if num_layers >= 6:
            self.conv6 = WaveletDownConvBlock(ngf*8, ngf*8, t_emb_dim = t_emb_dim, downsample=True,act=act)
